...
**2026-06-04**
- **STAB-1** (signal stability) — Swing/positional signals flickered LONG↔FLAT within a session. Cause: `price_fetcher.get_latest_daily` returns yfinance 1d data **including today's still-forming candle**, so daily-derived features (ADX/EMA spread/RSI/returns) + regime drifted every ~15-min cycle, and on a marginal model margin (e.g. INDUSIND swing L0.48/F0.51 = 3pp) the argmax flipped. The regime already guarded against this (3-bar consensus); the ML signal didn't. Also a train/inference skew — models train on CLOSED bars but were inferring on a partial one. Fix (user-chosen "closed candles for swing/positional"): `FeatureBuilder._closed_daily(df)` drops the last daily bar **iff the market is open AND the bar is today's** (after close it's complete → kept, so EOD still sees today); `build_all` routes the daily-derived FEATURES (tech_df, patterns, S/R, regime via `tech_df`) through the closed df, while **`current_price` and anomaly detection stay LIVE** (price must be real-time; anomaly must catch intraday shocks). Intraday (5m) features intentionally stay live — that timeframe is meant to update. Net: a multi-day signal now only changes when a new daily bar closes, not on the partial candle. Verified: market-open → drops today's bar (`06-04`→`06-03`); signal pipeline clean, `current_price` still live, R:R 2.5. Marginal day-to-day flips on genuinely-3pp-margin signals remain (legitimate — that's real model uncertainty, filtered by Gate 6 confidence). Code-only — `git pull && restart`.
