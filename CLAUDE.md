...
## ML Models — Latest CV (prod variant, retrained 2026-05-27, CR-1A regime fix)
...
- **NEWS-1** — VPS news `total=0` every cycle. Root cause: Google News IP-blocked (datacenter ASN) AND the ET per-bank topic RSS feeds now return an empty 776-byte shell (0 entries) → no bank-specific source left; generic feeds (moneycontrol/livemint/cnbc) work but rarely keyword-match the 5 banks. Fix: added **GDELT DOC 2.0 API** (`gdelt_query` per bank, `_gdelt_url`) as the datacenter-friendly per-bank PRIMARY in `_build_feeds` (works on VPS + locally, no key). Dead ET feeds dropped from the fetch (config field kept for re-enable). GDELT rate-limits bursts (429) → throttled ≥6s apart (`_throttle_gdelt`) + one 10s-backoff retry. Verified 75 entries/bank through the real fetch path. Gzip fix (VPS-1) confirmed already live on the VPS.
...
11. **Multi-bank tables MUST include `ticker`**: `feature_snapshots`, `options_snapshots`, `bse_announcements`, `fundamentals`, `regime_snapshots`, `open_trades`, `closed_trades`, `signal_outcomes`, `news`, `gate_results`.
12. **Live + training regime share `classify_regime_row`** (top of `feature_builder.py`) — change only the `_REGIME_*` constants. Reads ADX+EMA spread only (no `returns_20d`, CR-1A).
...
18. **VPS deployments must set `QE_IS_VPS=1`** — else Google News wastes cycles on 403 from DO IPs. On VPS the per-bank source is **GDELT** (`gdelt_query` in `news_fetcher.py`), not Google News / ET topic RSS (both dead from datacenter IPs). GDELT is throttled ≥6s/req — don't remove `_throttle_gdelt` or you'll 429.
19. **Intermarket peer/sector fetches are universe-cached** (`_intermarket_cache`, 5-min TTL). No per-bank yfinance calls there.
20. **Gate 5 S/R R:R hard floor = 0.5** (`RR_HARD_BLOCK`). Below 0.5:1 = hard block regardless of confidence. Soft penalty only 0.5–2.0. Don't lower.
...
26. **Gate 5 Grade D threshold category-aware** (`GRADE_D_OVERRIDE_CONF`: pos 68%, swing 65%, intra 62%). Never flatten to a single float. Pass `category=cat`.
27. **`_empty_sr()` returns Grade D / -0.2** — <30 bars = no info; Grade D is honest, C would silently promote no-data entries.
