...
G6-5 (open, minor) — Threshold boost reason always says "+DQ boost" regardless of actual cause
data_quality_boost carries ALL stacked boosts (counter-regime, HIGH_VOL, regime instability, fundamentals_stale, macro_stale, DQ). The reason string just says +DQ boost when the boost might actually be from a counter-regime trade. Misleading for debugging (cosmetic — doesn't affect gating).

Test hygiene (open) — 7 pre-existing unit-test failures are stale assertions, NOT code bugs: capital-mode tests assert SMALL/GROWING but `FORCE_CAPITAL_MODE=FULL` (paper override) returns FULL; vix_panic/earnings/circuit-breaker tests assert renamed strings. Update tests to set capital mode explicitly / match current reason strings before going live.
